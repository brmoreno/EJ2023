...
void draw(){
  d+=0.1;
  VectorC cen = new VectorC(color(255));
  for(int i = 0; i<fuente.pixels.length;i++){
    VectorC c = new VectorC(fuente.pixels[i]);
    c.rotate(d,cen);
    fuenteColor.pixels[i] = c.getColor();
  }
  fuenteColor.updatePixels();
  for(int i = 0; i<width; i+= 10){
    for(int j = 0; j<width; j+= 10){
      
      puntoEnZona(i+random(-t/2,t/2),j+random(-t/2,t/2),random(t/4,2*t));
    }
  }
}

void puntoEnZona(float x_, float y_, float t_){
  float r = 0;
  float g = 0;
  float b = 0;
  int x = ceil(x_);
  ...
  for(int i = 0 ; i<t_; i++){
    for(int j = 0 ; j<t_; j++){
      color c = fuente.get(x+i-ceil(t_/2),y+j-ceil(t_/2));
      if(ran<0.5){
        c = fuenteColor.get(x+i-ceil(t_/2),y+j-ceil(t_/2));
      }
      r+= red(c);
      g+= green(c);
      b+= blue(c);
